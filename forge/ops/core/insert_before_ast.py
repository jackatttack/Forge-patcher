# -*- coding: utf-8 -*-
"""
ops.insert_before_ast
=====================
AST insert-before target op.
"""
...
SPEC = {
    ...
    "summary": 'Insert a new method or function immediately before an existing AST target.',
}

# Runtime hint strings keyed by failure mode, surfaced by the hints engine.

HELP = {
    'summary': 'Insert a new function or method immediately before an existing resolved AST target.',
    'subject': [
        'Required. Existing AST target to insert before.',
    ...
    'related_ops': ['LIST_TARGETS', 'PREVIEW', 'INSERT_AFTER', 'INSERT_INTO', 'REPLACE'],
    'minimal_example': [
        'INSERT_BEFORE app.py::old_helper',
        'BEGIN_BODY',
        'def new_helper():',
        '    return True',
        'END_BODY',
...
def validate(parsed_op):
    ...
    if not (parsed_op.target or '').strip():
        errors.append('INSERT_BEFORE requires a target')
    if not parsed_op.body:
        errors.append('INSERT_BEFORE requires body content')
    return errors


def execute(ctx, parsed_op, result):
    """Resolve AST target and insert body immediately before its opening line."""
    ...
    result['message'] = 'inserted before %s lines %d-%d' % (
        resolved['kind'],
        resolved['start'],
        resolved['end'],
    )
