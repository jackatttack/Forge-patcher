# -*- coding: utf-8 -*-
"""
forge.extensions.linkos.render.cards
=====================================

Boxed and badged card primitives for LinkOS.
...
def action_link(label, *args, **kwargs):
    ...
    :func:`forge.extensions.linkos.render.pills.pill_link`.

    Keyword arguments:
        icon   Leading glyph (default ``🔗``).
        tone   Palette tone for the title (default ``accent``).
        note   Muted explanatory line under the title.
        tag    Optional uppercase ``[TAG]`` after the title.
        index  Either an integer (zero-padded ``01``) or arbitrary string.
    """
    ...
    # Visible button label first for captured stdout, then live link.
