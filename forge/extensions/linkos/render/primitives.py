# -*- coding: utf-8 -*-
"""
forge.extensions.linkos.render.primitives
==========================================

Theme, colour, low-level text helpers, and URL building for LinkOS.

This is the foundation module — everything else in ``render/`` and
every page imports from here. It owns:
...
def colour(name):
    """Set the console foreground colour by palette name. Silent no-op
    outside Pythonista."""
    if not console:
        return
    try:
        console.set_color(*PALETTE.get(name, PALETTE['text']))
    except Exception:
        pass
...
def section_label(text, icon=None, tone='muted'):
    ...
    Lighter than :func:`hero.big_section` — for lists where each item
    already carries its own visual weight.
    """
    if icon:
...
def write_tap(label, href, tone='border'):
    ...
    :func:`forge.extensions.linkos.render.pills.pill_link` when the
    full label can be the link.
    """
    colour(tone)
    print(label)
    reset()
